...
def schedule_s3_cleanup(older_than_hours: int = 24) -> str:
    ...
    try:
        job_id = redis_queue_service.enqueue_cleanup_task(
            task_type="s3_cleanup", older_than_hours=older_than_hours
        )

        logger.info(
            f"Limpeza S3 agendada - Job: {job_id}, older_than: {older_than_hours}h"
        )
        return job_id
...
def schedule_orphaned_cleanup() -> str:
    ...
    """
    try:
        job_id = redis_queue_service.enqueue_cleanup_task(task_type="orphaned_files")
...
def schedule_expired_cleanup() -> str:
    ...
    """
    try:
        job_id = redis_queue_service.enqueue_cleanup_task(task_type="expired_uploads")
...
def main():
    ...
    parser.add_argument("--s3-cleanup", action="store_true", help="Apenas limpeza S3")
    parser.add_argument(
        "--orphaned-files", action="store_true", help="Apenas limpeza de órfãos"
    )
    parser.add_argument(
        "--expired-uploads", action="store_true", help="Apenas limpeza de expirados"
    )
    parser.add_argument(
        "--older-than",
        type=int,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simular execução sem agendar jobs"
    )

    args = parser.parse_args()

    if not any(
        [args.daily, args.s3_cleanup, args.orphaned_files, args.expired_uploads]
    ):
        parser.print_help()
        sys.exit(1)
