...
class RealDocumentTester:
        ...
        try:
            ...
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    return {
                        "success": False,
                        "error": f"Presigned URL failed: {resp.status}",
                    }
            ...
            with open(file_path, "rb") as file_obj:
                ...
                async with session.post(upload_url, data=form_data) as resp:
                    if resp.status not in [200, 201, 204]:
                        ...
                        return {
                            "success": False,
                            "error": f"S3 upload failed: {resp.status}",
                        }

            print("   ✅ Upload S3 concluído")
            ...
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    return {
                        "success": False,
                        "error": f"Processing failed: {resp.status}",
                    }
    ...
    async def monitor_processing_jobs(
        self, session: aiohttp.ClientSession, jobs: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                try:
                    async with session.get(
                        ...
                        if status == "completed":
                            ...
                            job.update({"final_status": "failed", "error": error_msg})
                            completed_jobs.append(job)
                        else:
                            print(
                                f"   🔄 {filename}: {status} ({progress}%) - {current_step}"
                            )
                            still_pending.append(job)
    ...
    async def test_search_with_real_docs(self, session: aiohttp.ClientSession) -> None:
        ...
        for query in search_queries:
            ...
            try:
                search_payload = {"message": query, "session_id": None}

                async with session.post(
                    f"{self.api_base_url}/api/v1/chat/ask", json=search_payload
                ) as resp:
                    if resp.status != 200:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if failed_uploads:
                ...
                for job in failed_uploads:
                    print(
                        f"      - {job.get('filename', 'unknown')}: {job.get('error', 'unknown')}"
                    )
            ...
            completed_jobs = await self.monitor_processing_jobs(
                session, successful_uploads
            )

            # Estatísticas finais
            completed_success = [
                job for job in completed_jobs if job.get("final_status") == "completed"
            ]
            completed_failed = [
                job for job in completed_jobs if job.get("final_status") == "failed"
            ]
            completed_timeout = [
                job for job in completed_jobs if job.get("final_status") == "timeout"
            ]
