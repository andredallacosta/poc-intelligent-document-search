...
def create_redis_connection() -> Redis:
    ...
    return Redis.from_url(settings.get_redis_url(), decode_responses=False)
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Processar todas as filas disponíveis"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Logs detalhados (DEBUG level)"
    )
    parser.add_argument(
        "--burst",
        action="store_true",
    ...
    parser.add_argument("--name", type=str, help="Nome do worker (para identificação)")
    ...
    try:
        worker = Worker(queue_names, connection=redis_conn, name=worker_name)
