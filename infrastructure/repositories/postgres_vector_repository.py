...
from infrastructure.database.models import DocumentoEmbeddingModel, DocumentoChunkModel, DocumentoModel
...
class PostgresVectorRepository(VectorRepository):
    ...
    async def add_chunk_embedding(
        ...
        try:
            ...
            logger.error(f"Erro inesperado ao adicionar embedding para chunk {chunk_id}: {e}")
    ...
    async def search_similar_chunks(
        ...
        try:
            ...
            stmt = select(
                ...
                (1 - DocumentoEmbeddingModel.embedding.cosine_distance(query_vector)).label('similarity_score')
            ).select_from(
                DocumentoEmbeddingModel
                .join(DocumentoChunkModel, DocumentoEmbeddingModel.chunk_id == DocumentoChunkModel.id)
                .join(DocumentoModel, DocumentoChunkModel.documento_id == DocumentoModel.id)
            )
            
            # Aplica filtro de similaridade
            if similarity_threshold > 0:
                stmt = stmt.where(
                    (1 - DocumentoEmbeddingModel.embedding.cosine_distance(query_vector)) >= similarity_threshold
                )
            ...
            if metadata_filter:
                for key, value in metadata_filter.items():
                    stmt = stmt.where(
                        func.json_extract_path_text(DocumentoModel.meta_data, key) == value
                    )
            
            # Ordena por similaridade e limita resultados
            stmt = stmt.order_by(
                (1 - DocumentoEmbeddingModel.embedding.cosine_distance(query_vector)).desc()
            ).limit(n_results)
    ...
    async def delete_document_embeddings(self, document_id: UUID) -> int:
        ...
        try:
            ...
            logger.debug(f"Removidos {deleted_count} embeddings do documento {document_id}")
    ...
    async def embedding_exists(self, chunk_id: UUID) -> bool:
        ...
        try:
            ...
            logger.error(f"Erro ao verificar existência de embedding para chunk {chunk_id}: {e}")
            return False
    ...
    def _embedding_to_vector(self, embedding: Embedding) -> List[float]:
        ...
        if isinstance(embedding.vector, np.ndarray):
            ...
            raise DocumentProcessingError(f"Formato de embedding não suportado: {type(embedding.vector)}")
