...
class PostgresPrefeituraRepository(PrefeituraRepository):
        ...
        try:
            ...
            if "unique constraint" in str(e).lower():
                raise BusinessRuleViolationError(
                    f"Prefeitura com nome '{prefeitura.nome}' já existe"
                )
    ...
    async def update(self, prefeitura: Prefeitura) -> Prefeitura:
        ...
        try:
            stmt = (
                update(PrefeituraModel)
                .where(PrefeituraModel.id == prefeitura.id.value)
                .values(
                    nome=prefeitura.nome,
                    quota_tokens=prefeitura.quota_tokens,
                    tokens_consumidos=prefeitura.tokens_consumidos,
            ...
            if result.rowcount == 0:
                raise BusinessRuleViolationError(
                    f"Prefeitura {prefeitura.id} não encontrada"
                )
    ...
    async def exists(self, prefeitura_id: PrefeituraId) -> bool:
        ...
        stmt = select(func.count(PrefeituraModel.id)).where(
            PrefeituraModel.id == prefeitura_id.value
        )
        result = await self._session.execute(stmt)
        count = result.scalar()
        return count > 0
    ...
    async def exists_by_nome(self, nome: str) -> bool:
        ...
        stmt = select(func.count(PrefeituraModel.id)).where(
            PrefeituraModel.nome == nome.strip()
        )
        result = await self._session.execute(stmt)
        count = result.scalar()
        return count > 0
