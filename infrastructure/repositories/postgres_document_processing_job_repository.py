...
class PostgresDocumentProcessingJobRepository(DocumentProcessingJobRepository):
    ...
    async def save(self, job: DocumentProcessingJob) -> None:
        ...
        try:
            ...
            if existing:
                ...
                existing.content_hash_algorithm = job.content_hash.algorithm if job.content_hash else None
                existing.content_hash_value = job.content_hash.value if job.content_hash else None
                ...
                model = DocumentProcessingJobModel(
                    ...
                    content_hash_algorithm=job.content_hash.algorithm if job.content_hash else None,
                    content_hash_value=job.content_hash.value if job.content_hash else None,
            ...
            logger.info(f"DocumentProcessingJob salvo: {job.id} - Status: {job.status.value}")
    ...
    async def find_by_document_id(self, document_id: UUID) -> Optional[DocumentProcessingJob]:
        ...
        try:
            stmt = select(DocumentProcessingJobModel).where(DocumentProcessingJobModel.document_id == document_id)
            result = await self.session.execute(stmt)
            model = result.scalar_one_or_none()
            ...
            logger.error(f"Erro ao buscar DocumentProcessingJob por document_id {document_id}: {e}")
            return None
    
    async def find_by_upload_id(self, upload_id: UUID) -> Optional[DocumentProcessingJob]:
        """Busca job por upload_id"""
        try:
            stmt = select(DocumentProcessingJobModel).where(DocumentProcessingJobModel.upload_id == upload_id)
            result = await self.session.execute(stmt)
            model = result.scalar_one_or_none()
            ...
            logger.error(f"Erro ao buscar DocumentProcessingJob por upload_id {upload_id}: {e}")
            return None
    ...
    async def find_by_status(self, status: ProcessingStatus, limit: int = 100) -> List[DocumentProcessingJob]:
        ...
        try:
            stmt = select(DocumentProcessingJobModel).where(
                DocumentProcessingJobModel.status == status.value
            ).limit(limit)
            result = await self.session.execute(stmt)
            ...
            logger.error(f"Erro ao buscar DocumentProcessingJobs por status {status.value}: {e}")
            return []
    
    async def find_processing_jobs(self, limit: int = 100) -> List[DocumentProcessingJob]:
        """Busca jobs em processamento"""
        try:
            processing_statuses = ['extracting', 'checking_duplicates', 'chunking', 'embedding']
            stmt = select(DocumentProcessingJobModel).where(
                DocumentProcessingJobModel.status.in_(processing_statuses)
            ).limit(limit)
            result = await self.session.execute(stmt)
            models = result.scalars().all()
    ...
    async def delete(self, job_id: UUID) -> bool:
        """Remove job"""
        try:
            stmt = delete(DocumentProcessingJobModel).where(DocumentProcessingJobModel.id == job_id)
            result = await self.session.execute(stmt)
            await self.session.commit()
    ...
    def _model_to_entity(self, model: DocumentProcessingJobModel) -> DocumentProcessingJob:
        """Converte model SQLAlchemy para entidade de domínio"""
        content_hash = None
        if model.content_hash_algorithm and model.content_hash_value:
            content_hash = ContentHash(
                algorithm=model.content_hash_algorithm,
                value=model.content_hash_value
            )
