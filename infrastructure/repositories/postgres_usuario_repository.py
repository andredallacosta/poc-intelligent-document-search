...
class PostgresUsuarioRepository(UsuarioRepository):
    ...
    async def save(self, usuario: Usuario) -> Usuario:
        ...
        try:
            model = UsuarioModel(
                id=usuario.id.value,
                prefeitura_id=(
                    usuario.prefeitura_id.value if usuario.prefeitura_id else None
                ),
                nome=usuario.nome,
                email=usuario.email,
            ...
            if "unique constraint" in str(e).lower() and "email" in str(e).lower():
                raise BusinessRuleViolationError(
                    f"Usuário com email '{usuario.email}' já existe"
                )
    ...
    async def find_by_prefeitura_id(
        ...
        stmt = select(UsuarioModel).where(
            UsuarioModel.prefeitura_id == prefeitura_id.value
        )
    ...
    async def update(self, usuario: Usuario) -> Usuario:
        ...
        try:
            stmt = (
                update(UsuarioModel)
                .where(UsuarioModel.id == usuario.id.value)
                .values(
                    prefeitura_id=(
                        usuario.prefeitura_id.value if usuario.prefeitura_id else None
                    ),
    ...
    async def exists(self, usuario_id: UsuarioId) -> bool:
        ...
        stmt = select(func.count(UsuarioModel.id)).where(
            UsuarioModel.id == usuario_id.value
        )
        result = await self._session.execute(stmt)
        count = result.scalar()
        return count > 0
    ...
    async def exists_by_email(self, email: str) -> bool:
        ...
        stmt = select(func.count(UsuarioModel.id)).where(
            UsuarioModel.email == email.strip().lower()
        )
        result = await self._session.execute(stmt)
        count = result.scalar()
        return count > 0
    ...
    async def count_anonimos(self) -> int:
        ...
        stmt = select(func.count(UsuarioModel.id)).where(
            UsuarioModel.prefeitura_id.is_(None)
        )
        result = await self._session.execute(stmt)
        return result.scalar()
    ...
    def _model_to_entity(self, model: UsuarioModel) -> Usuario:
        ...
        return Usuario(
            ...
            prefeitura_id=(
                PrefeituraId.from_uuid(model.prefeitura_id)
                if model.prefeitura_id
                else None
            ),
            nome=model.nome,
            email=model.email,
