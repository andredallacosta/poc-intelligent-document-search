...
class PostgresUserRepository(UserRepository):
    ...
    async def save(self, user: User) -> User:
        ...
        try:
            model = UserModel(
                id=user.id.value,
                municipality_id=(
                    user.municipality_id.value if user.municipality_id else None
                ),
                name=user.name,
                email=user.email,
    ...
    async def find_by_municipality_id(
        ...
        stmt = stmt.order_by(UserModel.name)
    ...
    async def find_all_active(
        ...
        stmt = select(UserModel).where(UserModel.active.is_(True))
        stmt = stmt.order_by(UserModel.name)
    ...
    async def count_active(self) -> int:
        """Counts active users"""
        stmt = select(func.count(UserModel.id)).where(UserModel.active.is_(True))
        result = await self._session.execute(stmt)
        return result.scalar()
