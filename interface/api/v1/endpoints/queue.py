...
class CleanupTaskRequest(BaseModel):
    ...
    task_type: str = Field(..., description="Tipo da tarefa (s3_cleanup, orphaned_files, expired_uploads)")
    older_than_hours: int = Field(24, description="Para s3_cleanup: remover arquivos mais antigos que X horas")
...
async def get_specific_queue_info(queue_name: str):
    ...
    try:
        ...
        info = redis_queue_service.get_queue_info(queue_name)
        if not info:
            raise HTTPException(status_code=404, detail=f"Fila não encontrada: {queue_name}")
...
async def get_job_status(job_id: str):
    ...
    try:
        ...
        return JobStatusResponse(
            ...
            ended_at=job_status['ended_at'].isoformat() if job_status['ended_at'] else None,
...
async def cancel_job(job_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail=f"Job não encontrado ou não pode ser cancelado: {job_id}")
...
async def retry_job(job_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail=f"Job não encontrado ou não pode ser reprocessado: {job_id}")
        ...
        return {
            ...
            "queues": {
                "document_processing": {
                    ...
                    "failed": document_info.get('failed_count', 0) if document_info else 0
                },
                "cleanup_tasks": {
                    "available": bool(cleanup_info),
                    "length": cleanup_info.get('length', 0) if cleanup_info else 0,
                    "failed": cleanup_info.get('failed_count', 0) if cleanup_info else 0
                }
            },
