...
class PresignedUploadRequest(BaseModel):
    ...
    file_size: int = Field(..., gt=0, le=5368709120, description="Tamanho do arquivo em bytes (máx 5GB)")
    content_type: str = Field(..., description="Tipo MIME do arquivo")
    title: Optional[str] = Field(None, max_length=255, description="Título do documento")
    description: Optional[str] = Field(None, max_length=1000, description="Descrição do documento")
    tags: Optional[List[str]] = Field(default_factory=list, description="Tags do documento")
...
class ProcessDocumentRequest(BaseModel):
    ...
    upload_id: UUID = Field(..., description="ID do upload")
    file_hash: Optional[str] = Field(None, description="Hash SHA256 do arquivo (opcional)")
...
class DocumentStatus(BaseModel):
    ...
    processing_time: int = Field(..., ge=0, description="Tempo de processamento em segundos")
    s3_file_deleted: bool = Field(..., description="Se arquivo S3 foi removido")
    duplicate_of: Optional[UUID] = Field(None, description="ID do documento original se duplicata")
...
class DocumentSearchRequest(BaseModel):
    ...
    similarity_threshold: float = Field(0.7, ge=0.0, le=1.0, description="Limite de similaridade")
    document_ids: Optional[List[UUID]] = Field(None, description="IDs específicos de documentos")
    content_types: Optional[List[str]] = Field(None, description="Tipos de conteúdo para filtrar")
...
class DocumentStats(BaseModel):
    ...
    processing_stats: Dict[str, float] = Field(..., description="Estatísticas de processamento")
    by_type: Dict[str, int] = Field(..., description="Documentos por tipo de arquivo")
    by_content_type: Dict[str, int] = Field(..., description="Documentos por tipo de conteúdo")
