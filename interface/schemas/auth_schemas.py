...
class UserResponse(BaseModel):
    ...
    primary_municipality_id: Optional[str] = Field(None, description="ID da prefeitura principal")
    municipality_ids: List[str] = Field(default_factory=list, description="IDs das prefeituras")
...
class CreateUserRequest(BaseModel):
    ...
    role: str = Field(..., description="Role do usuário", pattern="^(superuser|admin|user)$")
    primary_municipality_id: str = Field(..., description="ID da prefeitura principal")
    municipality_ids: Optional[List[str]] = Field(None, description="IDs das prefeituras adicionais")
    auth_provider: str = Field("email_password", description="Provedor de autenticação", pattern="^(email_password|google_oauth2)$")
...
class ActivateUserRequest(BaseModel):
    ...
    password: Optional[str] = Field(None, min_length=6, description="Senha (obrigatória para email/senha)")
