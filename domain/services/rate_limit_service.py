...
class RateLimitService:
    ...
    def __init__(self, redis_client: RedisClient):
        ...
        self._limits = {
            ...
            "/api/v1/auth/login": {
                ...
                "per_email": {"count": 3, "window": 60},   # 3 tentativas por email por minuto
            },
            
            # Google OAuth2 - Menos restritivo (Google já protege)
            "/api/v1/auth/google": {
                "per_ip": {"count": 10, "window": 60},     # 10 tentativas por IP por minuto
            },
            
            # Chat - Moderado (protege custos OpenAI)
            "/api/v1/chat/ask": {
                "per_user": {"count": 20, "window": 60},   # 20 mensagens por usuário por minuto
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if current_count >= limit_config["count"]:
                retry_after = limit_config["window"] - (int(datetime.utcnow().timestamp()) % limit_config["window"])
    ...
    async def reset_rate_limit(self, endpoint: str, identifier: str, limit_type: str) -> bool:
        """
        ...
        """
        try:
            window_start = int(datetime.utcnow().timestamp()) // 60  # Assume janela de 60s
            key = f"rate_limit:{endpoint}:{limit_type}:{identifier}:{window_start}"
    ...
    async def get_rate_limit_status(
        ...
        try:
            ...
            current_count = await self._get_counter(key)
            reset_time = datetime.fromtimestamp((window_start + 1) * limit_config["window"])
