...
class DocumentService:
    ...
    async def create_document(
        ...
        if await self._document_repository.exists(metadata.source):
            raise DocumentAlreadyExistsError(f"Document with source '{metadata.source}' already exists")
    ...
    async def list_documents(self, limit: Optional[int] = None, offset: int = 0) -> List[Document]:
    ...
    async def add_chunks_to_document(self, document_id: UUID, chunks: List[DocumentChunk]) -> Document:
