...
class Prefeitura:
    ...
    def _validate_business_rules(self):
        ...
        if len(self.nome) > 255:
            raise BusinessRuleViolationError("Nome da prefeitura não pode ter mais de 255 caracteres")
        ...
        if self.tokens_consumidos > self.quota_tokens:
            raise BusinessRuleViolationError("Tokens consumidos não pode exceder a quota")
    ...
    def pode_consumir(self, quantidade: int) -> bool:
        ...
        return (
            self.ativo and 
            self.tokens_consumidos + quantidade <= self.quota_tokens
        )
