...
class DocumentProcessingJob:
    ...
    def _validate_business_rules(self):
        ...
        if self.chunks_processed > self.total_chunks and self.total_chunks > 0:
            raise BusinessRuleViolationError("Chunks processados não pode exceder o total")
        
        if self.processing_time_seconds < 0:
            raise BusinessRuleViolationError("Tempo de processamento não pode ser negativo")
    ...
    def update_chunks_progress(self, chunks_processed: int, total_chunks: int) -> None:
        ...
        if self.status == ProcessingStatus.EMBEDDING and total_chunks > 0:
            chunk_progress = (chunks_processed / total_chunks) * 30  # 30% da fase embedding
            base_progress = 55  # Progresso base até embedding
            self.progress = min(100, int(base_progress + chunk_progress))
            
            self.current_step = f"Gerando embeddings (batch {chunks_processed}/{total_chunks})..."
