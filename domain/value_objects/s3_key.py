...
class S3Key:
    ...
    def _validate(self):
        ...
        if len(self.bucket) > 63:
            raise BusinessRuleViolationError(
                "Bucket S3 não pode ter mais de 63 caracteres"
            )

        if len(self.key) > 1024:
            raise BusinessRuleViolationError(
                "Key S3 não pode ter mais de 1024 caracteres"
            )

    @classmethod
    def create_temp_key(
        cls, document_id: str, filename: str, bucket: str, region: str = "us-east-1"
    ) -> "S3Key":
    ...
    def url(self) -> str:
        """Retorna URL S3"""
        region_part = (
            f".{self.region}" if self.region and self.region != "us-east-1" else ""
        )
