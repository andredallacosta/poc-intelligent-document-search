...
class TestFlexibleUserActivationIntegration:
    ...
    async def test_check_invitation_token_valid(self, mock_container, invited_user):
        ...
        with patch('interface.dependencies.container.get_container', return_value=mock_container):
            app = FastAPI()
    ...
    @pytest.mark.asyncio
    async def test_activate_user_email_password_success(self, mock_container, invited_user):
        ...
        mock_container.get_user_management_use_case.return_value = mock_user_management_use_case
        ...
        with patch('interface.dependencies.container.get_container', return_value=mock_container):
            ...
            app.dependency_overrides[get_user_management_use_case] = lambda: mock_user_management_use_case
    ...
    @pytest.mark.asyncio
    async def test_activate_user_google_oauth2_success(self, mock_container, invited_user):
    ...
    @pytest.mark.asyncio
    async def test_activate_user_missing_password_for_email_provider(self, mock_container):
        ...
        with patch('interface.dependencies.container.get_container', return_value=mock_container):
            ...
            mock_use_case.activate_user_account = AsyncMock(
                side_effect=ValueError("Password é obrigatório para auth_provider email_password")
            )
            from interface.dependencies.container import get_user_management_use_case
            app.dependency_overrides[get_user_management_use_case] = lambda: mock_use_case
    ...
    async def test_activate_user_missing_google_token(self, mock_container):
        ...
        with patch('interface.dependencies.container.get_container', return_value=mock_container):
            ...
            mock_use_case = AsyncMock()
            mock_use_case.activate_user_account = AsyncMock(
                side_effect=ValueError("Google token é obrigatório para auth_provider google_oauth2")
            )
            from interface.dependencies.container import get_user_management_use_case
            app.dependency_overrides[get_user_management_use_case] = lambda: mock_use_case
    ...
    @pytest.mark.asyncio
    async def test_full_user_creation_and_activation_flow(self, mock_container, admin_user):
        ...
        with patch('interface.dependencies.container.get_container', return_value=mock_container):
            ...
            users_app.dependency_overrides[get_user_management_use_case] = lambda: mock_user_management_use_case
            ...
            with TestClient(auth_app) as client:
                check_response = client.get("/auth/check-invitation/new_invitation_token")
            ...
            auth_app.dependency_overrides[get_user_management_use_case] = lambda: mock_user_management_use_case
            
            with TestClient(auth_app) as client:
                activate_response = client.post("/auth/activate", json={
                    "invitation_token": "new_invitation_token",
                    "auth_provider": "google_oauth2",
                    "google_token": "google_token_123"
                })
