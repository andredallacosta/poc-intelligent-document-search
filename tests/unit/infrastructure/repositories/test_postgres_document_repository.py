...
class TestPostgresDocumentRepository:
    ...
    @pytest.mark.asyncio
    async def test_save_new_document_success(
        self, repository, mock_session, sample_document
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_existing_document_success(
        self, repository, mock_session, sample_document
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_document_integrity_error(
        self, repository, mock_session, sample_document
    ):
        # Mock para documento não existente
        mock_result = Mock()
    ...
    @pytest.mark.asyncio
    async def test_find_by_source_found(
        self, repository, mock_session, sample_document
    ):
        mock_model = Mock(spec=DocumentModel)
        mock_session.execute.return_value.scalar_one_or_none.return_value = mock_model
    ...
    @pytest.mark.asyncio
    async def test_find_by_title_similarity(
        self, repository, mock_session, sample_document
    ):
        mock_model = Mock(spec=DocumentModel)
        mock_result = Mock()
    ...
    @pytest.mark.asyncio
    async def test_find_by_content_search(
        self, repository, mock_session, sample_document
    ):
        mock_model = Mock(spec=DocumentModel)
        mock_result = Mock()
    ...
    @pytest.mark.asyncio
    async def test_find_by_content_hash_found(
        self, repository, mock_session, sample_document
    ):
        mock_model = Mock(spec=DocumentModel)
        mock_session.execute.return_value.scalar_one_or_none.return_value = mock_model
...
class TestPostgresDocumentChunkRepository:
    ...
    @pytest.mark.asyncio
    async def test_save_chunk_integrity_error(
        self, repository, mock_session, sample_chunk
    ):
        mock_session.flush.side_effect = IntegrityError("statement", "params", "orig")
        mock_session.rollback = AsyncMock()
        with pytest.raises(DocumentProcessingError):
    ...
    @pytest.mark.asyncio
    async def test_find_chunks_by_document_id_success(
        self, repository, mock_session, sample_chunk
    ):
        mock_model = Mock(spec=DocumentChunkModel)
        mock_result = Mock()
