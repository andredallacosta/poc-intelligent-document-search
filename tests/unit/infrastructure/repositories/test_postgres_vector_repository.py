...
class TestPostgresVectorRepository:
    ...
    @pytest.mark.asyncio
    async def test_add_chunk_embedding_success(self, repository, mock_session, sample_embedding):
        chunk_id = uuid4()
        
        # Mock chunk exists
        with patch.object(repository, '_chunk_exists', return_value=True):
            with patch.object(repository, '_delete_chunk_embedding_internal', return_value=None):
                with patch.object(repository, '_embedding_to_vector', return_value=[0.1] * 1536):
                    mock_session.flush = AsyncMock()
                    
                    result = await repository.add_chunk_embedding(chunk_id, sample_embedding)
    ...
    @pytest.mark.asyncio
    async def test_add_chunk_embedding_chunk_not_found(self, repository, mock_session, sample_embedding):
        chunk_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_add_chunk_embedding_integrity_error(self, repository, mock_session, sample_embedding):
        chunk_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_search_similar_success(self, repository, mock_session, sample_embedding):
        # Mock search results
        mock_row = Mock()
        mock_row.id = uuid4()
        ...
        with patch.object(repository, '_embedding_to_vector', return_value=[0.1] * 1536), \
             patch.object(repository, '_vector_to_embedding', return_value=sample_embedding):
            results = await repository.search_similar_chunks(sample_embedding, n_results=5, similarity_threshold=0.8)
    ...
    @pytest.mark.asyncio
    async def test_search_similar_empty(self, repository, mock_session, sample_embedding):
        mock_result = Mock()
    ...
    async def test_get_embedding_by_chunk_id_found(self, repository, mock_session, sample_embedding):
        ...
        with patch.object(repository, '_vector_to_embedding', return_value=sample_embedding):
    ...
    @pytest.mark.asyncio
    async def test_count_embeddings_success(self, repository, mock_session):
        mock_result = Mock()
    ...
    @pytest.mark.asyncio
    async def test_delete_chunk_embedding_internal_success(self, repository, mock_session):
        chunk_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_search_with_filters_success(self, repository, mock_session, sample_embedding):
        ...
        with patch.object(repository, '_embedding_to_vector', return_value=[0.1] * 1536), \
             patch.object(repository, '_vector_to_embedding', return_value=sample_embedding):
            results = await repository.search_similar_chunks(sample_embedding, n_results=5, similarity_threshold=0.8, metadata_filter=filters)
