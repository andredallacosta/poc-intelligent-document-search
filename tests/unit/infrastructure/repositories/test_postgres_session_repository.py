...
class TestPostgresSessionRepository:
    ...
    async def test_save_session_success(
        self, repository, mock_session, sample_chat_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_session_integrity_error(
        self, repository, mock_session, sample_chat_session
    ):
    ...
    async def test_find_session_by_id_found(
        ...
        mock_model.user_id = (
            sample_chat_session.user_id.value if sample_chat_session.user_id else None
        )
        ...
        with patch.object(
            repository, "_model_to_entity", return_value=sample_chat_session
        ):
    ...
    @pytest.mark.asyncio
    async def test_find_sessions_by_user_success(
        self, repository, mock_session, sample_chat_session
    ):
        mock_model = Mock(spec=ChatSessionModel)
        mock_result = Mock()
    ...
    @pytest.mark.asyncio
    async def test_deactivate_session_success_duplicate(self, repository, mock_session):
        mock_result = Mock()
        mock_result.rowcount = 1
        mock_session.execute.return_value = mock_result
    ...
    @pytest.mark.asyncio
    async def test_deactivate_session_not_found_duplicate(
...
class TestPostgresMessageRepository:
    ...
    @pytest.mark.asyncio
    async def test_save_message_integrity_error(
        self, repository, mock_session, sample_message
    ):
        mock_session.flush.side_effect = IntegrityError("statement", "params", "orig")
        mock_session.rollback = AsyncMock()
        with pytest.raises(SessionNotFoundError):
    ...
    @pytest.mark.asyncio
    async def test_find_message_by_id_found(
        self, repository, mock_session, sample_message
    ):
