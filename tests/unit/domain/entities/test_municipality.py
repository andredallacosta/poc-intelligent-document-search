...
class TestMunicipality:
    ...
    def test_create_municipality_factory_method(self):
        municipality = Municipality.create("Rio de Janeiro Municipality", token_quota=5000)
    ...
    def test_empty_name_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Municipality name is required"):
    ...
    def test_name_too_long_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Municipality name cannot exceed 255 characters"):
            Municipality(id=municipality_id, name=long_name, token_quota=10000)
    
    def test_negative_quota_raises_error(self):
        municipality_id = MunicipalityId.generate()
        
        with pytest.raises(BusinessRuleViolationError, match="Token quota cannot be negative"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=-1000)
    
    def test_negative_tokens_consumed_raises_error(self):
        municipality_id = MunicipalityId.generate()
        
        with pytest.raises(BusinessRuleViolationError, match="Tokens consumed cannot be negative"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=10000, tokens_consumed=-100)
    
    def test_tokens_consumed_exceeds_quota_raises_error(self):
        municipality_id = MunicipalityId.generate()
        
        with pytest.raises(BusinessRuleViolationError, match="Tokens consumed cannot exceed quota"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=10000, tokens_consumed=15000)
    
    def test_negative_monthly_limit_raises_error(self):
        municipality_id = MunicipalityId.generate()
        
        with pytest.raises(BusinessRuleViolationError, match="Monthly limit must be positive"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=10000, monthly_token_limit=0)
    
    def test_monthly_limit_too_high_raises_error(self):
        municipality_id = MunicipalityId.generate()
        
        with pytest.raises(BusinessRuleViolationError, match="Monthly limit cannot exceed 1M tokens"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=10000, monthly_token_limit=1000001)
    ...
    def test_future_contract_date_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Contract date cannot be in the future"):
            Municipality(id=municipality_id, name="Test Municipality", token_quota=10000, contract_date=future_date)
    ...
    def test_consume_negative_tokens_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Token amount must be positive"):
    ...
    def test_increase_quota_below_consumed_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="New quota.*cannot be less than already consumed tokens"):
    ...
    def test_update_monthly_limit_negative_raises_error(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="New limit must be positive"):
            municipality.update_monthly_limit(0)
    
    def test_update_monthly_limit_too_high_raises_error(self):
        municipality = Municipality.create("Test Municipality", token_quota=10000)
        
        with pytest.raises(BusinessRuleViolationError, match="Limit cannot exceed 1M tokens"):
