...
class TestContentHash:
    ...
    def test_invalid_algorithm(self):
        with pytest.raises(BusinessRuleViolationError, match="Algoritmo deve ser 'sha256' ou 'md5'"):
    ...
    def test_empty_hash_value(self):
        with pytest.raises(BusinessRuleViolationError, match="Hash value é obrigatório"):
    ...
    def test_sha256_wrong_length_short(self):
        with pytest.raises(BusinessRuleViolationError, match="Hash SHA256 deve ter 64 caracteres"):
    ...
    def test_md5_wrong_length_short(self):
        with pytest.raises(BusinessRuleViolationError, match="Hash MD5 deve ter 32 caracteres"):
    ...
    def test_non_hexadecimal_characters(self):
        invalid_hash = "g" * 64
        with pytest.raises(BusinessRuleViolationError, match="Hash deve conter apenas caracteres hexadecimais"):
    ...
    def test_from_text_unsupported_algorithm(self):
        with pytest.raises(BusinessRuleViolationError, match="Algoritmo não suportado: sha1"):
