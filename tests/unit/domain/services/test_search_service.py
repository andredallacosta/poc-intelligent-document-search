...
class TestSearchService:
    ...
    @pytest.mark.asyncio
    async def test_search_similar_content_with_threshold_service(self, mock_vector_repository, sample_embedding):
        ...
        mock_threshold_service.get_threshold_for_query.assert_called_once_with("test with threshold service")
        mock_vector_repository.search_similar_chunks.assert_called_once_with(
            query_embedding=sample_embedding,
            n_results=3,
    ...
    @pytest.mark.asyncio
    async def test_search_similar_content_default_threshold(self, search_service, mock_vector_repository, sample_embedding):
        mock_vector_repository.search_similar_chunks = AsyncMock(return_value=[])
    ...
    @pytest.mark.asyncio
    async def test_search_similar_content_exception_handling(self, search_service, mock_vector_repository, sample_embedding):
        from domain.exceptions.chat_exceptions import SearchError
        
        mock_vector_repository.search_similar_chunks = AsyncMock(side_effect=Exception("Database error"))
    ...
    def test_filter_and_rank_results(self, search_service, mock_data_factory):
        ...
        filtered_results = search_service._filter_and_rank_results(results, threshold=0.5)
    ...
    def test_filter_and_rank_results_all_below_threshold(self, search_service, mock_data_factory):
        results = []
    ...
    def test_convert_results_to_references_with_metadata(self, search_service, mock_data_factory):
        from domain.repositories.vector_repository import SearchResult
        
        results = []
        for i in range(2):
            chunk = mock_data_factory.create_document_chunk(content=f"Chunk {i} content")
            result = SearchResult(
                chunk=chunk,
    ...
    def test_convert_results_to_references_no_metadata(self, search_service, mock_data_factory):
        from domain.repositories.vector_repository import SearchResult
    ...
    def test_convert_results_to_references_partial_metadata(self, search_service, mock_data_factory):
        from domain.repositories.vector_repository import SearchResult
