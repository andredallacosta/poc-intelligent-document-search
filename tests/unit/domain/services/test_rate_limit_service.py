...
class TestRateLimitService:
    ...
    async def test_check_rate_limit_within_limit(
        self, rate_limit_service, mock_redis_client
    ):
        """Test rate limit check when within limit"""
        # Mock Redis responses
        mock_redis_client.redis.get.return_value = "2"  # Current count
        mock_redis_client.redis.pipeline.return_value.__aenter__.return_value.execute = (
            AsyncMock()
        )
        # Check rate limit
        is_allowed, retry_after = await rate_limit_service.check_rate_limit(
            endpoint="/api/v1/auth/login", identifier="192.168.1.1", limit_type="per_ip"
        )
    ...
    @pytest.mark.asyncio
    async def test_check_multiple_limits_success(
        self, rate_limit_service, mock_redis_client
    ):
        ...
        await rate_limit_service.check_multiple_limits(
            ...
            checks={"per_ip": "192.168.1.1", "per_email": "test@example.com"},
        )

    @pytest.mark.asyncio
    async def test_check_multiple_limits_exceeded(
        self, rate_limit_service, mock_redis_client
    ):
    ...
    def test_get_limit_config(self, rate_limit_service):
        ...
        config = rate_limit_service._get_limit_config(
            "/api/v1/auth/login", "per_unknown"
        )
        assert config is None

    @pytest.mark.asyncio
    async def test_increment_counter_error_handling(
        self, rate_limit_service, mock_redis_client
    ):
        """Test increment counter handles Redis errors gracefully"""
        # Mock Redis pipeline to raise exception
        mock_redis_client.redis.pipeline.side_effect = Exception(
    ...
    async def test_chat_endpoint_limits(self, rate_limit_service, mock_redis_client):
        ...
        is_allowed, retry_after = await rate_limit_service.check_rate_limit(
            endpoint="/api/v1/chat/ask", identifier="user123", limit_type="per_user"
        )
        assert is_allowed is True
        assert retry_after is None
