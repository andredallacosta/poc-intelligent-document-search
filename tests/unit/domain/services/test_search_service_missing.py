...
class TestSearchServiceMissingCoverage:
    ...
    @pytest.mark.asyncio
    async def test_search_similar_content_with_threshold_filtering(
        self, search_service, mock_vector_repository, sample_search_results
    ):
        query_embedding = Embedding.from_openai([0.1, 0.2, 0.3])
        mock_vector_repository.search_similar_chunks.return_value = (
    ...
    def test_convert_results_to_references_missing_metadata(self, search_service):
        ...
        search_result = SearchResult(
            chunk=chunk, similarity_score=0.7, distance=0.3, metadata={}
        )
    ...
    async def test_search_similar_content_exception_handling(
        self, search_service, mock_vector_repository
    ):
        query_embedding = Embedding.from_openai([0.1, 0.2, 0.3])
        mock_vector_repository.search_similar_chunks.side_effect = Exception(
            "Database error"
        )
        with pytest.raises(Exception) as exc_info:
            await search_service.search_similar_content(
                query="exception test", query_embedding=query_embedding
            )
        assert "Failed to search similar content" in str(exc_info.value)

    def test_create_excerpt_long_content(self, search_service):
        long_content = (
            "This is a very long content that should be truncated because it exceeds the maximum "
            "length limit that we set for excerpts in our search results to keep them readable and concise."
        )
    ...
    def test_calculate_relevance_score(self, search_service):
        ...
        score = search_service.calculate_relevance_score(
            0.8, document_popularity=1.2, recency_factor=0.9
        )
