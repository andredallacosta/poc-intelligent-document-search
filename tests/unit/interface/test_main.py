...
class TestLifespan:
    ...
    @pytest.mark.asyncio
    async def test_lifespan_startup_success(self, mock_settings, mock_container, mock_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_lifespan_redis_connection_failed(self, mock_settings, mock_container, mock_db_connection):
        mock_redis_client = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_lifespan_redis_exception(self, mock_settings, mock_container, mock_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_lifespan_database_connection_failed(self, mock_settings, mock_container):
        mock_redis_client = AsyncMock()
        mock_redis_client.ping = AsyncMock(return_value=True)
        mock_container.get_redis_client.return_value = mock_redis_client
...
class TestEndpoints:
    ...
    def test_app_info_endpoint(self, client, mock_settings):
        ...
        assert data["features"]["contextual_retrieval"] == mock_settings.use_contextual_retrieval
        assert data["limits"]["max_messages_per_session"] == mock_settings.max_messages_per_session
