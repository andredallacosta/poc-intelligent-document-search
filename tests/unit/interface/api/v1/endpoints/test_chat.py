...
from interface.api.v1.endpoints.chat import (
    ask_question,
...
class TestChatEndpoints:
    ...
    def sample_chat_request(self):
        return ChatRequest(
            message="What is the capital of France?", session_id=None, metadata={}
        )
    ...
    async def test_ask_question_success(
        self, sample_chat_request, sample_chat_response_dto, mock_chat_use_case
    ):
        with patch(
            "interface.dependencies.container.create_chat_use_case",
            return_value=mock_chat_use_case,
        ):
            mock_chat_use_case.execute = AsyncMock(
                return_value=sample_chat_response_dto
            )
    ...
    @pytest.mark.asyncio
    async def test_ask_question_with_session_id(
        self, mock_chat_use_case, sample_chat_response_dto
    ):
        session_id = uuid4()
        request = ChatRequest(
    ...
    async def test_ask_question_generic_chat_error(self, mock_chat_use_case):
        ...
        with patch(
            ...
            mock_chat_use_case.execute = AsyncMock(
                side_effect=ChatError("Generic chat error")
            )
            with pytest.raises(HTTPException) as exc_info:
                await ask_question(request)
