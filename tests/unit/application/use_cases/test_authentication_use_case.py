...
class TestAuthenticationUseCase:
    ...
    @pytest.mark.asyncio
    async def test_login_email_password_success(self, auth_use_case, mock_auth_service, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_login_email_password_invalid_credentials(self, auth_use_case, mock_auth_service):
    ...
    @pytest.mark.asyncio
    async def test_login_email_password_user_not_found(self, auth_use_case, mock_auth_service):
    ...
    @pytest.mark.asyncio
    async def test_login_google_oauth2_success(self, auth_use_case, mock_auth_service, google_user):
        """Deve fazer login com Google OAuth2 com sucesso"""
        # Arrange
        request = LoginGoogleOAuth2DTO(
    ...
    @pytest.mark.asyncio
    async def test_login_google_oauth2_user_not_found(self, auth_use_case, mock_auth_service):
        ...
        mock_auth_service.authenticate_google_oauth2 = AsyncMock(
            side_effect=UserNotFoundError("Usuário não encontrado. Solicite convite ao administrador.")
        )
    ...
    @pytest.mark.asyncio
    async def test_verify_token_success(self, auth_use_case, mock_auth_service, sample_user):
        ...
        assert user_dto.municipality_ids == [str(mid.value) for mid in sample_user.municipality_ids]
    ...
    @pytest.mark.asyncio
    async def test_verify_token_user_without_municipality(self, auth_use_case, mock_auth_service):
        ...
        token = "valid_jwt_token"
        mock_auth_service.verify_jwt_token = AsyncMock(return_value=user_without_municipality)
    ...
    @pytest.mark.asyncio
    async def test_verify_token_user_without_last_login(self, auth_use_case, mock_auth_service):
    ...
    @pytest.mark.asyncio
    async def test_login_email_password_dto_validation(self, auth_use_case, mock_auth_service, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_login_google_oauth2_dto_validation(self, auth_use_case, mock_auth_service, google_user):
    ...
    @pytest.mark.asyncio
    async def test_user_dto_serialization_comprehensive(self, auth_use_case, mock_auth_service):
