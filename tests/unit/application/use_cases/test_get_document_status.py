...
class TestGetDocumentStatusUseCase:
    ...
    async def test_execute_document_not_found(self, use_case, mock_job_repository):
        ...
        with pytest.raises(
            BusinessRuleViolationError, match=f"Documento não encontrado: {document_id}"
        ):
    ...
    async def test_execute_repository_exception(self, use_case, mock_job_repository):
        document_id = uuid4()
        mock_job_repository.find_by_document_id = AsyncMock(
...
class TestGetJobStatusUseCase:
    ...
    async def test_execute_job_not_found(self, use_case, mock_job_repository):
        ...
        with pytest.raises(
            BusinessRuleViolationError, match=f"Job não encontrado: {job_id}"
        ):
    ...
    @pytest.mark.asyncio
    async def test_execute_checking_duplicates_status(
    ...
    @pytest.mark.asyncio
    async def test_execute_with_estimated_time_remaining(
        self, use_case, mock_job_repository
    ):
        job = DocumentProcessingJob(
            id=uuid4(),
        ...
        result = await use_case.execute(job.id)
        assert (
            result.estimated_time_remaining is not None
            or result.estimated_time_remaining is None
        )

    @pytest.mark.asyncio
    async def test_execute_repository_exception(self, use_case, mock_job_repository):
        job_id = uuid4()
        mock_job_repository.find_by_id = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_execute_with_all_fields_populated(
        self, use_case, mock_job_repository
    ):
        job = DocumentProcessingJob(
            id=uuid4(),
