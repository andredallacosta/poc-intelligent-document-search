...
from application.use_cases.process_uploaded_document import ProcessUploadedDocumentUseCase
from application.dto.document_dto import ProcessDocumentRequestDTO, ProcessDocumentResponseDTO
...
class TestProcessUploadedDocumentUseCase:
    ...
    async def test_execute_success_new_job(
        ...
        mock_file_upload_repository.find_by_id = AsyncMock(return_value=sample_file_upload)
        ...
        with patch('application.use_cases.process_uploaded_document.redis_queue_service') as mock_redis:
        ...
        mock_file_upload_repository.find_by_id.assert_called_once_with(valid_request.upload_id)
        mock_job_repository.find_by_upload_id.assert_called_once_with(valid_request.upload_id)
    ...
    async def test_execute_upload_not_found(
        ...
        with pytest.raises(BusinessRuleViolationError, match=f"Upload não encontrado: {valid_request.upload_id}"):
    ...
    async def test_execute_upload_no_s3_key(
        ...
        with pytest.raises(BusinessRuleViolationError, match="Upload não possui chave S3"):
            await use_case.execute(valid_request)
    ...
    async def test_execute_upload_expired(
        ...
        with patch('domain.entities.file_upload.FileUpload.is_expired', new_callable=lambda: property(lambda self: True)):
    ...
    async def test_execute_redis_enqueue_failure(
        ...
        with patch('infrastructure.queue.redis_queue.redis_queue_service') as mock_redis:
            mock_redis.enqueue_document_processing.side_effect = Exception("Redis connection failed")
            
            with pytest.raises(BusinessRuleViolationError, match="Falha ao iniciar processamento"):
                await use_case.execute(valid_request)
    ...
    async def test_execute_repository_error(
        ...
        mock_file_upload_repository.find_by_id = AsyncMock(side_effect=Exception("Database error"))
    ...
    def test_validate_upload_state_valid(self, use_case, sample_file_upload):
        with patch('domain.entities.file_upload.FileUpload.is_expired', new_callable=lambda: property(lambda self: False)):
