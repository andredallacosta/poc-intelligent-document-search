...
def upgrade() -> None:
    ...
    op.create_table('documento',
        ...
        sa.Column('meta_data', postgresql.JSON(astext_type=sa.Text()), server_default='{}'),
        sa.Column('criado_em', sa.DateTime(timezone=True), server_default=sa.text('now()')),
        sa.Column('atualizado_em', sa.DateTime(timezone=True), server_default=sa.text('now()')),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index('idx_documento_metadata_source', 'documento', [sa.text("json_extract_path_text(meta_data, 'source')")])
    op.create_index('idx_documento_source_unique', 'documento', [sa.text("json_extract_path_text(meta_data, 'source')")], unique=True)
    op.create_index('idx_documento_titulo', 'documento', ['titulo'])
    
    # Create documento_chunk table
    op.create_table('documento_chunk',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('documento_id', 'indice_chunk', name='unique_chunk_per_document')
    )
    op.create_index('idx_chunk_documento', 'documento_chunk', ['documento_id'])
    op.create_index('idx_chunk_indice', 'documento_chunk', ['documento_id', 'indice_chunk'])
    
    # Create documento_embedding table
    op.create_table('documento_embedding',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('chunk_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('embedding', Vector(1536), nullable=False),
        sa.Column('criado_em', sa.DateTime(timezone=True), server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['chunk_id'], ['documento_chunk.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_embedding_chunk', 'documento_embedding', ['chunk_id'], unique=True)
    ...
    op.create_table('message',
        ...
        sa.Column('referencias_documento', postgresql.JSON(astext_type=sa.Text()), server_default='[]'),
        sa.Column('tokens_usados', sa.Integer(), server_default='0'),
        sa.Column('meta_data', postgresql.JSON(astext_type=sa.Text()), server_default='{}'),
        sa.Column('criado_em', sa.DateTime(timezone=True), server_default=sa.text('now()')),
        sa.CheckConstraint("role IN ('user', 'assistant', 'system')", name='check_valid_role'),
    ...
    op.execute('CREATE INDEX idx_documento_embedding_vector ON documento_embedding USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)')
