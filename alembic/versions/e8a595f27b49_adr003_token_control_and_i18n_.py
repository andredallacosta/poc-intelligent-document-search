...
def upgrade() -> None:
    ...
    op.alter_column('municipality', 'tokens_consumidos', new_column_name='tokens_consumed')
    ...
    op.alter_column('message', 'referencias_documento', new_column_name='document_references')
    ...
    op.add_column('municipality', sa.Column('monthly_token_limit', sa.Integer(), nullable=False, server_default='20000'))
    op.add_column('municipality', sa.Column('contract_date', sa.Date(), nullable=False, server_default=sa.text('CURRENT_DATE')))
    
    # Add constraints for new fields
    op.create_check_constraint('check_monthly_limit_positive', 'municipality', 'monthly_token_limit > 0')
    
    # === STEP 5: CREATE TOKEN_USAGE_PERIOD TABLE ===
    
    op.create_table('token_usage_period',
        ...
        sa.CheckConstraint('extra_credits >= 0', name='check_extra_credits_non_negative'),
        sa.CheckConstraint('tokens_consumed >= 0', name='check_tokens_consumed_non_negative'),
        sa.CheckConstraint('period_start < period_end', name='check_period_valid'),
        sa.CheckConstraint('tokens_consumed <= (base_limit + extra_credits)', name='check_tokens_not_exceed_limit'),
        
        # Unique constraint
        sa.UniqueConstraint('municipality_id', 'period_start', name='uq_municipality_period'),
    ...
    op.create_index('idx_token_period_current', 'token_usage_period', ['municipality_id', 'period_start', 'period_end'])
    op.create_index('idx_token_period_active', 'token_usage_period', ['municipality_id', 'period_end'])
...
def downgrade() -> None:
    ...
    op.alter_column('message', 'document_references', new_column_name='referencias_documento')
    ...
    op.alter_column('prefeitura', 'tokens_consumed', new_column_name='tokens_consumidos')
