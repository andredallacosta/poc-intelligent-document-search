...
def upgrade() -> None:
    ...
    op.add_column('user', sa.Column('full_name', sa.String(length=255), nullable=False, server_default=''))
    op.add_column('user', sa.Column('role', sa.String(length=20), nullable=False, server_default='user'))
    op.add_column('user', sa.Column('primary_municipality_id', sa.UUID(), nullable=True))
    op.add_column('user', sa.Column('municipality_ids', sa.JSON(), nullable=True, server_default='[]'))
    op.add_column('user', sa.Column('auth_provider', sa.String(length=20), nullable=False, server_default='email_password'))
    ...
    op.add_column('user', sa.Column('invitation_token', sa.String(length=255), nullable=True))
    ...
    op.create_foreign_key('fk_user_primary_municipality', 'user', 'municipality', ['primary_municipality_id'], ['id'])
    ...
    op.create_index('idx_user_primary_municipality', 'user', ['primary_municipality_id'])
    op.create_index('idx_user_role_active', 'user', ['role', 'is_active'])

    # Add check constraints
    op.create_check_constraint('check_role_valid', 'user', "role IN ('superuser', 'admin', 'user')")
    op.create_check_constraint('check_auth_provider_valid', 'user', "auth_provider IN ('email_password', 'google_oauth2')")
    op.create_check_constraint('check_email_password_has_hash', 'user', 
                              "(auth_provider = 'email_password' AND password_hash IS NOT NULL) OR (auth_provider = 'google_oauth2')")
    op.create_check_constraint('check_google_oauth_has_id', 'user',
                              "(auth_provider = 'google_oauth2' AND google_id IS NOT NULL) OR (auth_provider = 'email_password')")
    op.create_check_constraint('check_invitation_token_has_expiry', 'user',
    ...
    op.execute("UPDATE \"user\" SET primary_municipality_id = municipality_id WHERE municipality_id IS NOT NULL")
    op.execute("UPDATE \"user\" SET municipality_ids = CASE WHEN municipality_id IS NOT NULL THEN json_build_array(municipality_id::text) ELSE '[]'::json END")
