...
class ProcessUploadedDocumentUseCase:
    ...
    async def execute(
        self, request: ProcessDocumentRequestDTO
    ) -> ProcessDocumentResponseDTO:
        """
        ...
        try:
            ...
            if existing_job:
                logger.info(
                    f"Job já existe para upload {request.upload_id}: {existing_job.id}"
                )
            ...
            try:
                ...
                logger.info(
                    f"Documento enfileirado no Redis - Job: {redis_job_id}, ProcessingJob: {job.id}"
                )
                ...
                await self.job_repository.save(job)
                raise BusinessRuleViolationError(
                    f"Falha ao iniciar processamento: {str(e)}"
                )
